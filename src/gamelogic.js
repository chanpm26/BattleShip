...
beginButton.addEventListener("click", function () {
  ...
  playerOne.gameBoard.placeShip(
    "battleship",
    ships.shipTwo.length,
    ships.shipTwo
  );
  playerOne.gameBoard.placeShip(
    "submarine",
    ships.shipThree.length,
    ships.shipThree
  );
  playerOne.gameBoard.placeShip(
    "cruiser",
    ships.shipFour.length,
    ships.shipFour
  );
  playerOne.gameBoard.placeShip(
    "destroyer",
    ships.shipFive.length,
    ships.shipFive
  );
});
...
playerTwo.gameBoard.placeShip(
  "carrier",
  5,
  computerCoordinates.firstShipCoordinates
);
playerTwo.gameBoard.placeShip(
  "battleship",
  4,
  computerCoordinates.secondShipCoordinates
);
playerTwo.gameBoard.placeShip(
  "submarine",
  3,
  computerCoordinates.thirdShipCoordinates
);
playerTwo.gameBoard.placeShip(
  "cruiser",
  3,
  computerCoordinates.fourthShipCoordinates
);
playerTwo.gameBoard.placeShip(
  "destroyer",
  2,
  computerCoordinates.fifthShipCoordinates
);
    ...
    if (lastComputerMove != "") {
      ...
      setTimeout(() => {
        ...
        while (move == "notvalid") {
          ...
          let randomTile = document.querySelector(
            `[data-id="${randomSquare}"]`
          );
          move = playGameHelper(
            currentPlayer,
            playerOne,
